                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-8">
                            ...
                            <div class="single-item-options">
                                <form action="{{route('dat-hang',$product->id )}}" method="post">
                                    {{csrf_field()}}
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <div class="single-item">
                                    @if ($related_product->promotion_price != 0)
                                        <div class="ribbon-wrapper"><div class="ribbon sale">Sale</div></div>
                                    @endif
                                    <div class="single-item-header">
                                        <a href="product.html"><img src="image/product/{{$related_product->image}}" height="250px" alt=""></a>
                                    </div>
                                    <div class="single-item-body">
                                        <p class="single-item-title">{{$related_product->name}}</p>
                                        <p class="single-item-price">
                                            @if($related_product->promotion_price == 0)
                                                <span class="flash-sale">{{number_format($related_product->unit_price)}} đồng</span>
                                            @else
                                                <span class="flash-del">{{number_format($related_product->unit_price)}} đồng</span>
                                                <span class="flash-sale">{{number_format($related_product->promotion_price)}} đồng</span>
                                            @endif
                                        </p>
                                    </div>
                                    <div class="single-item-caption">
                                        <a class="add-to-cart pull-left" href="{{route('dat-hang',$related_product->id )}}"><i class="fa fa-shopping-cart"></i></a>
                                        <a class="beta-btn primary" href="{{route('san-pham',$related_product->id)}}">Chi tiết <i class="fa fa-chevron-right"></i></a>
                                        <div class="clearfix"></div>
                                    </div>
                                </div>
                            </div>
